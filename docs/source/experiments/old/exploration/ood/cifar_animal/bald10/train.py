...
def main(acq_name, b, iters, repeats, seed=42):
    ...
    train, test = FilterDataset(train, animal_classes), FilterDataset(test, animal_classes)
    ...
    for r in range(1, REPEATS + 1):
        ...
        model = MCDropout(CIFAR10Net(num_classes=len(animal_classes)), forward=20, fast=False).to(device)
        if acq_name == "bald":
