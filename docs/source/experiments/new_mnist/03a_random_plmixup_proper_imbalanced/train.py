...
def main(alpha: float,
    ...
    mapping[4] = 1
    mapping[7] = 1
    train, test = Dataset.MNIST.get(raw=True)
    ...
    pool = make_imbalanced_pool(pool, [2, 4, 7], n=500)
    pool_idxs = (*pool_idxs, pool.indices)
    class_count = defaultdict(int)
    for _, y in pool:
        class_count[y] += 1
    ...
    if augment:
        data_augmentation = Dataset.MNIST.get_augmentation
    else:
    ...
    for r in range(1, REPEATS + 1):
        ...
        model = Net(Dataset.MNIST.model).to(device)
