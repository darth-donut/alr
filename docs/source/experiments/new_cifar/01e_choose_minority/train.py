...
def main(model_name, aug, dataset, iters, repeats, result, minority):
    manual_seed(42)
    mcs = ', '.join(map(str, minority))
    print(f"Minority classes: {mcs}")
    ...
    template = f"{model_name}_{dataset}" + ("_aug" if aug else "") + "_" + '_'.join(map(str, minority))
    ...
    with open(metrics / "subset_idxs.pkl", "wb") as fp:
        ...
        for i in range(1, ITERS + 1):
            ...
            with timeop() as t:
                for m in range(5):
                    ...
                    models.append(model)
            print(f"Trained 5 models, time: {t}")
            # eval
...
if __name__ == '__main__':
    ...
    args.add_argument("--config", type=int, help=f"Integer between [0, {N-1}], inclusive")
    args = args.parse_args()

    manual_seed(115)
    subset = np.random.choice(range(120), size=N, replace=False)
    choices = np.array(list(itertools.combinations(range(10), 3)))[subset]
    minority = choices[args.config].tolist()

    main("13cnn", args.aug, args.data, args.iters, args.reps, result=f"scores/13cnn_{'_'.join(map(str, minority))}", minority=minority)
