...
from alr.training.utils import _accuracy, _expected_calibration_error, _confidence_threshold, _entropy
from alr.utils import savefig

root = Path("/Users/harry/Documents/workspace/thesis/experiments/vanilla_repeated_acquisition/mnist/permanent/pl_metrics/no_al_b=10_thresh=0.9/repeat_4")
...
def reliability_hist_plot(bins_E_M, counts_E_N, axis, cmap: str = 'viridis',
...
reliability_hist_plot(bins, counts, axes[0], xticklabels=False, title="(Permanent) Reliability plot")
reliability_plot(bins, bin_accuracy, counts, axes[1], title=None)
norm = mpl.colors.Normalize(vmin=1, vmax=accuracies.shape[0])
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=mpl.cm.get_cmap('viridis')),
             orientation='vertical', label="Epoch", cax=axes[2])
savefig("/Users/harry/Documents/workspace/thesis/figures/3/perm_reliability_plot.pdf")
perm_ece = ece

root = Path("/Users/harry/Documents/workspace/thesis/experiments/vanilla_repeated_acquisition/mnist/reconsider/pl_metrics/no_al_b=10_thresh=0.9/repeat_1")
files = sorted(root.glob("*.pkl"), key=lambda x: int(str(x.name).split("_")[0]))
confidences, proportions, accuracies = [], [], []
bins, bin_accuracy, counts, ece = [], [], [], []
entropy = []
...
spec = fig.add_gridspec(ncols=2, nrows=2, width_ratios=[29, 1], height_ratios=[2, 7], )
axes = [fig.add_subplot(spec[0, 0]), fig.add_subplot(spec[1, 0]), fig.add_subplot(spec[:, -1])]
reliability_hist_plot(bins, counts, axes[0], xticklabels=False, title="(Ephemeral) Reliability plot")
reliability_plot(bins, bin_accuracy, counts, axes[1], title=None)
norm = mpl.colors.Normalize(vmin=1, vmax=accuracies.shape[0])
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=mpl.cm.get_cmap('viridis')),
             orientation='vertical', label="Epoch", cax=axes[2])
