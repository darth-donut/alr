...
root = Path(f"/Users/harry/Documents/workspace/thesis/reports/10_proper_imbalanced_class/data_files/bald10_imbalanced/metrics/")


# Plot the classes acquired by imbalanced BALD
def ridge_plot_discrete(distributions: np.array,
                        title: str, xlabel: str, ylabel: str,
                        cmap="cet_bmy_r",
                        figsize=(8, 6), every=1, colour=True,
    ...
    if colour:
        [i.set_color("red") for idx, i in enumerate(ax_objs[-1].get_xticklabels()) if idx in {0, 4, 9}]
    plt.tight_layout()
...
for seed in seeds:
    ...
    ridge_plot_discrete(
        class_dist, title="Acquired class distribution over time",
        xlabel="Class", ylabel="Dataset size",
        every=5, idx_transform=itc,
    )
    savefig(
...
[i.set_color("red") for idx, i in enumerate(axes[1].get_xticklabels()) if idx in {0, 4, 9}]
