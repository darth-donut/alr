...
def main(model_name, aug, dataset, iters, repeats, result, minority, seed):
    manual_seed(42)
    mcs = ', '.join(map(str, minority))
    print(f"Seed {seed} with minority classes: {mcs}")
    ...
    template = f"{model_name}_{dataset}" + ("_aug" if aug else "") + "_" + '_'.join(map(str, minority)) + "_" + str(seed)
...
if __name__ == '__main__':
    ...
    minority = [2, 4, 6]
    main("13cnn", args.aug, args.data, args.iters, args.reps, result=f"scores/13cnn_{'_'.join(map(str, minority))}_{args.seed}", minority=minority, seed=args.seed)
