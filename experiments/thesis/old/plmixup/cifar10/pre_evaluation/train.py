...
def main(acq_name: str, alpha: float, b: int, augment: bool, iters: int, repeats: int):
    manual_seed(42)
    ...
    for r in range(1, REPEATS + 1):
        ...
        for i in range(1, ITERS + 1):
            ...
            torch.save(
                model_regular.state_dict(), saved_models / f"rep_{r}_iter_{i}.pt"
            )
            ...
            with timeop() as t:
                history = trainer.fit(
                    train_loader, val_loader, epochs=EPOCHS_SUPERVISED
                )
            ...
            with dm.unlabelled.tmp_debug():
                ...
                calc_calib_metrics(
                    pool_loader,
                    model,
                    calib_metrics / "supervised" / "pool" / f"rep_{r}" / f"iter_{i}",
                    device=device,
                )
            calc_calib_metrics(
                test_loader,
                model,
                calib_metrics / "supervised" / "test" / f"rep_{r}" / f"iter_{i}",
                device=device,
            )
            ...
            torch.save(
                model.state_dict(), saved_models / f"supervised_rep_{r}_iter_{i}.pt"
            )
