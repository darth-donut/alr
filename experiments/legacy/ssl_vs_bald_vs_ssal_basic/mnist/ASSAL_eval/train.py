r"""
This came from the question: how do we know if SSL training had improved the acquisition's
performance? Hard to say what samples are optimal for the current model. what we can do instead is
compare the accuracies to a regular BALD experiment that doesn't run SSL at al. However, this also means
that the only time we are using SSL here is BEFORE acquisition. The model is then reset to its original
weights and trained using supervised labels only.

WARNING: saved_models and metrics are 10-ahead. The save is done after acquiring which was a mistake.
So if the file says dsize_n, it's actually for a model trained on dsize n - 10.
"""
...
def main(acq_name, b, iters, log_every):
    ...
    for r in range(1, REPEATS + 1):
        ...
        for i in range(1, ITERS + 1):
            ...
            test_metric = sup_trainer.evaluate(test_loader)
            print(f"\t[test] eval acc: {test_metric['acc']}")
            eval_accs[dm.n_labelled].append(test_metric['acc'])
            ...
            print(f"\t[test] SSAL acc: {test_metric['acc']}; time: {t}")
    ...
    with open(f"{acq_name}_eval_accs_{b}.pkl", "wb") as fp:
        pickle.dump(eval_accs, fp)
