...
def main(optim='SGD', alpha=1, batch_size=100,
         min_label_prop=16, rfls_len=20_000, val_size=5_000,
         warm_up_epochs=10, epochs=400, start_size=1000,
         dropout=0.1, swa=False, augment=True, jitter=True):
    assert not jitter or augment
    root = Path("results")
    ...
    save_pl_metrics.run(torchdata.DataLoader(ds, batch_size=512, shuffle=False, **kwargs))


if __name__ == '__main__':
    args = argparse.ArgumentParser()
    args.add_argument("--start_size", default=1000, type=int)
    args.add_argument("--jitter", action='store_true')
    args = args.parse_args()
    main(dropout=0.5, augment=True, alpha=1, start_size=args.start_size, jitter=args.jitter)
