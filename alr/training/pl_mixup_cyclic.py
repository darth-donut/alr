...
from alr.training.pl_mixup import IndexMarker, PDS, onehot_transform, create_warmup_trainer, create_plmixup_trainer, \
    PLMixupTrainer
...
def cyclic_annealer(t, T, M, init_lr=0.2):
    return (init_lr / 2) * (np.cos(
        (np.pi * np.mod(t - 1, math.ceil(T / M))) / math.ceil(T / M)
    ) + 1)
